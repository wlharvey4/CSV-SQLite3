...
{{{heading(The ~ledger~ ~convert~ Command)}}}
...
The Zero Ledger File is a ~ledger~ file with directives that associate =payee=
's with =account= 's.  It is used by the ~ledger~ {{{command(convert)}}}
command to parse a CSV file into the ~ledger~ format.  It uses two directives,
each of which uses a sub-directive, of the form:
...
In the above, the first line establishes a legitimate payee.  The second line
is a regular expression from the =payee= line that is associated with that payee.
