...
be accomplished with a Perl script concatenating the files, sorting the lines,
and unique'ifying them and then saving and backing up new and original files.
...
1. Download raw data from a bank by account, start date, and end date, which
   ...
   #+begin_src perl -n :tangle catuniq.pl
     ...
     # Backup original $dest_file to $WORKBAK
   ...
   #+begin_src sh
   ln -s $PWD/catuniq.pl $WORKBIN/catuniq
   #+end_src
