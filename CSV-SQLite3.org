...
come in from my bank through CSV and QFX downloads.  There is a =Download
Transactions= button that loads a modal window with settings for
...
dates or all dates, etc. without accumulating duplicate data.
...
2. Process the raw data to remove extraneous and unnecessary data, and reformat
   the data into the proper form and columns for efficiently working with the
   financial data, e.g., producing accounting and tax reports, budgets, etc..
   This processed data will be stored in an SQLite3 database, avoiding
   duplicating data at all times.
