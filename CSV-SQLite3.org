...
#+author:LOLH
#+date:<2019-10-20 Sun 11:34>
#+macro: version Version 0.0.81
  ...
  (function check() {
      for (env in exports) {
          if (typeof exports[env] === 'undefined') {
              console.error(`ERROR: Environment variable ${env} is not defined. Halting execution.`);
              process.exit(1);
          }
      }
  })();
...
** Install Package.json and Dependencies
...
processes the commndline arguments and dispatches to the appropriate points,
...
#+begin_src js +n :tangle index.js
  ...
  /* ATTACH OR DELETE */
  let db;
  if (options.hasOwnProperty('attach') || options.hasOwnProperty'delete') {
      const option = options.attach ? 'attach' : 'delete';
      const value = options.attach ? options.attach : options.delete;
      db = require('./lib/sql/db')[option](value);
  }
...
#+begin_src js -n :tangle lib/sql/db.js
    ...
    exports.attach = function (db_file) {
        ...
        const WORKDB = ENV.WORKDB;         // base directory for .sqlite db files
        ...
        const ({db_path, csv_path}) = format_paths(db_file);
    ...
    exports.delete = function (db_file) {
        ...
        const WORKBAK_LEDGER = path.format({ // base directory for .ledger files
            dir:   WORKBAK,
            name: 'ledger'
        });
        ...
        const ({db_path}) = format_paths(db_file);
...
#+begin_src js -n :tangle lib/csv/file_transformer.js
  ...
  exports.transformer = require('csv').transform(transform_fn);
  const db      = require('./lib/sql/db');
  const db_data = require('./lib/sql/definitions');
...
#+begin_src js -n :tangle lib/csv/file_parser.js
  ...
  parser.on('readable', function() {
      ...
      while (record = parser.read()) {
          transform.write(record); // continue down the rabbit hole
      }
  }).on('error', function(err) {
      console.error(err.message); 
...
#+begin_src js -n :tangle lib/csv/source_stream.js
  ...
  module.exports = function(csv_info) { // csv_info = options.csv = [acct, year]
      ...
      try {
          return csv_stream = fs.createReadStream(
