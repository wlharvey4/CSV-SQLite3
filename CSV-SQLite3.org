...
#+begin_src js +n :tangle index.js
  ...
  if (options.hasOwnProperty('delete')) {
      ...
      try {
          ...
          if (err.code === 'ENOENT')
              console.log(`file ${db_path} did not exist; ignoring.`);
          else {
              throw err;
          }
      }
...
#+begin_src js +n :tangle index.js
  ...
  if (options.hasOwnProperty('export')) {
      ...
      const fd = fs.openSync(`${WORKDB}/${export_csv}.csv`,'w');
      ...
      const ret = spawnSync(
          ...
          [
              ...
              '-csv',
              db_path,
              'SELECT rowid,date,trans,checkno,txfr,payee,category,memo,desc1,desc2,caseno,amount FROM usb;'
          ],
          {
              encoding: 'utf-8',
              stdio: [0,fd,1]}
