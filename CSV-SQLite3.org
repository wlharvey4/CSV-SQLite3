...
#+author:LOLH
#+date:<2019-10-20 Sun 10:25>
#+macro: version Version 0.0.80
...
#+begin_src js +n :tangle index.js
  ...
  if (!options.hasOwnProperty('attach')) {
      options.attach = DB_DEFAULT;
  }
  options.attach = path.format({
      path: ENV.WORKDB,
      name: options.attach,
      ext: '.sqlite3'
  });
  console.log(`db file path is: ${options.attach}`);
...
#+caption:CSV-SQLite3 Process CSV Files
