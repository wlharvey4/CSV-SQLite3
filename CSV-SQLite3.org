...
#+begin_src js +n :tangle index.js
  ...
  const transform_function = function (record) {
      // Format date as yyyy-mm-dd and delete Date
      record.date = new Date(record['Date']).toISOString().split('T')[0];
      delete record['Date'];

      // Change Transaction to type
      record.type = record['Transaction'];
      delete record['Transaction'];

      // Change Name to payee
      record.payee = record['Name'];
      delete record['Name'];

      // Change Amount to amount as Currency type
      record.amount = accounting.formatMoney(record['Amount']);
      delete record['Amount'];

      // Clean up Memo by removing Download message; return as memo
      let re = new RegExp('Download from usbank.com.\\s*');
      record.memo = record['Memo'].replace(re,'');
      delete record['Memo'];

      re = /INTERNET BANKING TRANSFER WITHDRAWAL/;
      if ((f = re.exec(record.payee))) {
          record.payee = record.payee.replace(re,'');
      }
