...
#+date:2019-10-06 19:56
#+macro: version Version 0.0.50
...
#+begin_src sh
  ...
  sed -i '' -e 's/\"version\": \"1.0.0\",/\"version\": \"0.0.49\",/' package.json
  yarn add command-line-args command-line-usage csv sqlite3 accounting
  yarn add https://github.com/wlharvey4/WLParser/archive/V0.1.tar.gz
#+end_src
...
{{{subheading(Find and save all checks into the SQLite3 database)}}}

: csvsqlite [--attach <db>] --find-checks <year> | -f <year>
...
#+begin_src js +n :tangle index.js
  const option_defs = [
      ...
      { name: 'find-checks', alias: 'f', type: Number, description: 'Find checks in worklog <year> and save to sqlite3 database file.' },
...
elements in the ~worklog~ files.  This package is set up to find and return all
checks.  At this point, this program will present an option to gather all
checks and place them into the SQLite3 database in a document called =checks=.
When this program converts the CSV files into the =ledger= format, it can do a
search for a particular check number in this SQLite3 document and incorporate
that data into the ledger file.

Use the option ~--find-checks <year>~ to find and store checks from the worklog
yearly file identified by =<year>= in the SQLite3 database in a document called
=checks.=

#+name:csv-sqlite3-find-checks-option
#+BEGIN_SRC js +n :tangle index.js
  if (options.hasOwnProperty('find-checks')) {
    console.log("Let's find some checks");
    process.exit(0);
  }
#+END_SRC
