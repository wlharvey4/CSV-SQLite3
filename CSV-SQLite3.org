...
: csv-sqlite3 [--attach <db>] --find-checks <year> | -f <year>
...
return all checks for a particular year.  At this point, this program will
present an option to gather all checks and place them into the SQLite3 database
in a table called =checks=.  When this program converts the CSV files into the
=ledger= format, it can do a search for a particular check number in this
SQLite3 document and incorporate that data into the ledger file.

Use the option ~--checks~ to find and store checks from the worklog yearly file
identified by =<year>= (from the {{{option(--csv)}}} option) in the
~worklog.<year>.otl~ file and save into the SQLite3 database =<db>= in a table
called =checks=.

#+name:csv-sqlite3-find-checks-option
#+header: :noweb yes
#+BEGIN_SRC js +n :tangle index.js
  if (options.hasOwnProperty('checks')) {
    console.log("Let's find some checks");
    db.run(`CREATE TABLE IF NOT EXISTS
      <<define-checks-table-schema>>`);

    const wlchecks = new WLChecks(wl_year);
    let done = false;
    wlchecks.on('check', check => {
      console.log(`checkno: ${check.checkno}`);
    }).on('checked', () => {
      console.log('Checked');
      done = true;
    });
