...
#+author:LOLH
#+date:<2019-10-19 Sat 12:03>
#+macro: version Version 0.0.76
...
#+header: :mkdirp yes
#+begin_src js :tangle lib/sql/definitions.js
  usb (
...
This section deals with creating the CSV-SQLite3 project and identifying its
dependencies.  It also deals with the project's main ~index.js~ file.

** The Project's Dependencies
{{{heading(External Dependencies)}}}

#+cindex:external dependencies
#+cindex:dependencies, external
...
- ~accounting~ :: http://openexchangerates.github.io/accounting.js/
                  provides simple and advanced number, money and currency
                  formatting.
- ~wlparser~ :: https://git@github.com/wlharvey4/wlparser.git
              provides ~worklog~ reading and parsing capabilities, particular
              for obtaining check information, for example


{{{heading(Node.js Internal Dependencies)}}}

#+cindex:internal dependencies
#+cindex:dependencies, external
...
** Create the CSV-SQLite3 Package and Dependencies

#+cindex:@file{CSV-SQLite3} project
#+cindex:@file{csv-sqlite3} package, dependencies
#+cindex:@file{package.json}, @file{csv-sqlite3}
Create the CSV-SQLite3 project by initializing a ~package.json~ and installing
its dependencies:
...
#+caption:Create the CSV-SQLite3 Project
...
** Create the Package ~index.js~ File and Import its Dependencies
:PROPERTIES:
:header-args: :comments both
:END:

#+cindex:@file{index.js}
After the ~csv-sqlite3~ package has been created and installed, create the
package's ~index.js~ file and import the project's dependencies.
...
#+begin_src js -n :tangle lib/csv/file_stringifier.js
  ...
  module.exports = function([acct, year]) {
      ...
      const {DB_COLS} = require('../sql/definitions');
      ...
      console.log(`usb_acct_year: ${usb_acct_year}`);
...
#+begin_src js +n :tangle lib/csv/file_transformer.js
  ...
  const stringifier  = require('./lib/csv/file_stringifier')(options.csv);
  const transform_fn = require('./lib/csv/transform_function');
...
#+begin_src js -n :tangle lib/csv/file_parser.js
  ...
  const transformer = require('./lib/csv/file_transformer');
  exports.parser    = require('csv').parse({columns: true});
...
#+begin_src js -n :tangle lib/csv/source_stream.js
  ...
  try {
      exports.csv_stream = fs.createReadStream(validate_data(exports.csv), {encoding: 'utf8'});
  } catch (err) {
      console.error(err.message);
      process.exit(1);
  }
  ...
  function validate_data ([acct, year]) {
      /* 'acct' and 'year' come from the command-line option '--csv'
          acct = options.csv[0]
          year = options.csv[1] */
      const WORKUSB = process.env.WORKUSB;
      if (!WORKUSB) throw new ReferenceError('You must create a valid WORKUSB environment variable.');
  ...
  #+begin_src js +n :tangle index.js
    if (options.hasOwnProperty('csv')) {
        ...
        const csv_source_stream = require('./lib/csv/source_stream');
        const csv_file_parser   = require('./lib/csv/file_parser');
    
        const csv_file_stream   = csv_source_stream(options.csv);
        csv_file_stream.pipe(csv_file_parser);
