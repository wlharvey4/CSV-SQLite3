...
future, I will focus solely on the years 2016-2019.  New data (transactions)
come in from my bank through CSV and QFX downloads.  I will download and save
the QFX data, but work exclusively with the CSV files at this point.  There is
a =Download Transactions= button that loads a modal window with settings for
...
So I can easily pick the account, start date, end date, and format, download
new transactions into a separate file.  The problem that arises is that because
this is a manual operation, it becomes difficult to keep track of what dates
have been downloaded and processed already, or even wanting to reprocess some
dates or all dates, etc. without accumulating duplicate data.  An acceptable
solution is to concatenate newly downloaded data into files according to
account and year, then sort the files and remove duplicate items.  This would
be accomplished with a Bash script concatenating the files, then piping data
through such programs as ~perl~ to format the date properly, then ~sort~, then
~uniq~, then into a new file.

{{{heading(A Workflow)}}}

A potential workflow would therefore be:

1. Download raw data from a bank by account, start date, and end date, and
   store in source files by account and year, avoiding duplicate data at all
   times.  The data is exported by the bank into files called ~export.csv~ and
   ~export.qfx~ in the default ~Download~ directory.  A script could review the
   data to determine what the year is.  However, there is no indication in
   either the filename or the data from which account the data was downloaded.
   Therefore, any script that is run must be told what account this data is
   associated with, as for example here:
   ...
   account.  It will create any necessary directory and file to make the
   command run.
   ...
   environment variable =$WORKSOURCES= if the ~--dest~ option is omitted.

   #+name:catuniq-source
   #+header: :shebang "#!/usr/bin/env perl"
   #+begin_src perl -n :tangle catuniq
     use strict;
     ...
     use OptArgs;			# Integrated argument and option processing
     use Data::Dumper;		# Stringified perl data structures

     $OptArgs::COLOUR = 1;
     $OptArgs::PRINT_DEFAULT = 1;
     $OptArgs::PRINT_ISA = 1;
     ...
     opt dest => (
         ...
         default => exists $ENV{WORKSOURCES} ? File::Spec->catdir($ENV{WORKSOURCES}, 'usb') : undef,
     );
     ...
     # verify $dest
     die ("STOP: you did not supply a '-dest' option and \$WORKSOURCES is not defined.") unless exists $opts->{dest};
     my $dest = File::Spec->catdir($opts->{dest}, $opts->{acct});

     # Find and verify 'export.csv'
     my $export = File::Spec->catfile($ENV{HOME}, 'Downloads', 'export.csv');
     -e -r -w $export || die("STOP: $export does not exist or is not readable or is not writable!!");
     ...
     File::Path::make_path($dest, {verbose => 1}) unless ( -d $dest );

     say("dest: $dest");
     say("cat from $export\n");

     # Find year from within export.csv
     open (LINE, "cat $export |")
         || die("Can't cat $export");
