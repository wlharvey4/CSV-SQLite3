...
#+author:LOLH
#+date:<2019-10-17 Thu 22:56>
#+macro: version Version 0.0.70
...
#+begin_src sh
  ...
  sed -i '' -e 's/\"version\": \"1.0.0\",/\"version\": \"0.0.70\",/' package.json
...
In order to keep the ~package.json~ =version= number in sync with this
document's version number, create a little script to update its version based
upon the macro version's current value.  This little script will then be run
whenever an installation occurs.
